...
class WrappedBlock(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.controller is not None:
            ...
            if len(controller.shape) == 1:
                controller = controller.reshape(1, 1, -1)
            assert len(controller.shape) == len(modified.shape), f"Shape of controller {controller.shape} does not match shape of modified {modified.shape}."

            controller = controller.to(modified.device)
            if type(mask) == torch.Tensor:
                mask = mask.to(modified.device)
            if isinstance(self.token_pos, int):
                modified[:, self.token_pos] = self.operator(modified[:, self.token_pos], controller * mask)
                ...
                if self.token_pos == "end":
                    len_token = controller.shape[1]
                    modified[:, -len_token:] = self.operator(modified[:, -len_token:], controller * mask)
    ...
    def set_controller(self, activations, token_pos=None, masks=None, normalize=False, operator='linear_comb'):
        ...
        self.controller = activations.squeeze() if activations.dim() <= 2 else activations
        self.mask = masks
...
class WrappedReadingVecModel(torch.nn.Module):
    ...
    def wrap(self, layer_id, block_name):
        ...
        if self.is_wrapped(self.model.model.layers[layer_id]):
            if hasattr(self.model.model.layers[layer_id].block, block_name):
            ...
            if hasattr(self.model.model.layers[layer_id], block_name):
    ...
    def get_activations(self, layer_ids, block_name='decoder_block'):
        ...
        def _get_activations(layer_id, block_name):
            ...
            if self.is_wrapped(current_layer):
                ...
                elif block_name in BLOCK_NAMES and hasattr(current_block, block_name) and self.is_wrapped(getattr(current_block, block_name)):
                    return getattr(current_block, block_name).output
                else:
                    assert False, f"No wrapped block named {block_name}."

            else:
                if block_name in BLOCK_NAMES and hasattr(current_layer, block_name) and self.is_wrapped(getattr(current_layer, block_name)):
    ...
    def set_controller(self, layer_ids, activations, block_name='decoder_block', token_pos=None, masks=None, normalize=False, operator='linear_comb'):
        ...
        def _set_controller(layer_id, activations, block_name, masks, normalize, operator):
            ...
            if block_name == 'decoder_block':
                ...
                current_block = current_layer.block
                if block_name in BLOCK_NAMES and hasattr(current_block, block_name) and self.is_wrapped(getattr(current_block, block_name)):
    ...
    def reset(self):
        for layer in self.model.model.layers:
            if self.is_wrapped(layer):
                layer.reset()
                for block_name in BLOCK_NAMES:
                    if hasattr(layer.block, block_name) and self.is_wrapped(getattr(layer.block, block_name)):
            ...
            for block_name in BLOCK_NAMES:
                ...
                if hasattr(layer, block_name):
                    ...
                    if self.is_wrapped(block):
                        setattr(layer, block_name, block.block)
